...
class DashboardNextDnsTest(unittest.TestCase):
    ...
    def test_dashboard_ux_copy_uses_clearer_local_loading_and_selection_states(self):
        ...
        self.assertIn("Latest Sample", html)
        self.assertIn("Observation Window", html)
    ...
    def test_dashboard_data_freshness_fields_match_between_desktop_and_mobile(self):
        html = (ROOT / "viz" / "index.html").read_text()

        self.assertIn('<h4><span>Data Freshness</span><span class="pill neutral" id="telemetryBadge">—</span></h4>', html)
        self.assertIn('<div class="metric-pair"><span class="label">Latest Sample</span><span class="metric-value" id="telemetrySummary">—</span></div>', html)
        self.assertIn('<div class="metric-pair"><span class="label">Observation Window</span><span class="metric-value" id="telemetryUpdated">24 hours</span></div>', html)
        self.assertIn('<h4>Data Freshness</h4>', html)
        self.assertIn('<div class="detail-line"><b>Status:</b> <span id="mobileTelemetryState">—</span></div>', html)
        self.assertIn('<div class="detail-line"><b>Latest Sample:</b> <span id="mobileTelemetrySummary">—</span></div>', html)
        self.assertIn('<div class="detail-line"><b>Observation Window:</b> <span id="mobileTelemetryUpdated">24 hours</span></div>', html)
        self.assertIn('document.getElementById("mobileTelemetryState").textContent = document.getElementById("telemetryState").textContent;', html)
        self.assertIn('document.getElementById("mobileTelemetrySummary").textContent = document.getElementById("telemetrySummary").textContent;', html)
        self.assertIn('document.getElementById("mobileTelemetryUpdated").textContent = document.getElementById("telemetryUpdated").textContent;', html)
        self.assertIn("function toCompactRelativeAge(d){", html)
        self.assertIn('let summary = toCompactRelativeAge(last.t);', html)
        self.assertIn('updatedEl.textContent = `${WINDOW_HOURS} hours`;', html)
